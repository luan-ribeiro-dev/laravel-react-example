...
import { storeBook } from '../../../api/requests/admin/books';
...
const mapDispatchToProps = {dispatchStoreBook: storeBook}
const connector = connect(mapStateToProps, mapDispatchToProps)
type ReduxProps = ConnectedProps<typeof connector>
type Props = ReduxProps

function CreateBook({storeBookState, dispatchStoreBook}: Props) {
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (storeBookState.status !== APIConstants.STARTED && storeBookState.status !== APIConstants.SUCCEEDED) {
      const data = {
        title,
        description,
        author,
      ...
      const filteredData = Object.fromEntries(Object.entries(data).filter(([_, v]) => v !== '' && v !== 0))

      dispatchStoreBook(filteredData)
    }
  }

  useEffect(() => {
    if (storeBookState.status === APIConstants.SUCCEEDED) {
